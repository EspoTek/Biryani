...
int usbInterface::setup(int bInterface){
    ...
    if(handle!=NULL){
        qDebug() << "usbInterface::setup() called, but interface is already set up!";
        return 0;
    }
    ...
    if(error){
        qFatal("Count not initialise libusb.  Error code %08x", error);
        return error;
    }
    ...
    if(handle==NULL){
        qFatal("Count not open device %04x/%04x.\nIs the headbox connected and preinitialised?\nIs the driver installed?", vid, pid);
        return -69; //Error: handle null
    }

    error = libusb_claim_interface(handle, bInterface);
    if(error){
        qFatal("Count not claim interface.  Error code %08x", error);
        return error;
    }
...
int usbInterface::transfer_control(bool dir_is_in, const unsigned char *expected_data, unsigned char *buffer, unsigned char bmRequestType, unsigned char bRequest, unsigned short wValue, unsigned short wIndex, unsigned short wLength){
    ...
    qDebug("libusb_control_transfer(handle, %02x, %02x, %04x, %04x, buffer, %04x, %u)", bmRequestType, bRequest, wValue, wIndex, wLength, timeout);
...
int usbInterface::transfer_bulk(bool dir_is_in, unsigned char endpoint, const unsigned char *expected_data, unsigned char *buffer, unsigned int data_length, int *transferred){
    ...
    qDebug("libusb_bulk_transfer(handle, 0x%02x, buffer, %u, transferred, %u);", (dir_is_in ? (0x80 | endpoint) : endpoint), data_length, timeout);
